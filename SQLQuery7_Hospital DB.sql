...
select patient_billing.name as [patient name],Doctors.name as [doctor name],patient_billing.bill_date,patient_billing.amount,patient_billing.status from 
...
-------------------------------------------------------------------------------------------------------------------------
-----------------9. Find Patients Who Have Had Appointments With More Than One Doctor
INSERT INTO Appointments (patient_id, doctor_id, appointment_date, appointment_time, status) 
VALUES 
...
---------------10.Get a List of Doctors and Their Total Appointments in a month of octobar

select DO.name,count(APP.doctor_id) as [Total Appointments] from Doctors as DO inner join Appointments as APP
on DO.doctor_id=APP.doctor_id
where format(APP.appointment_date,'MMMM')='October'
group by DO.name
-----------11.Retrieve Total Income Generated From Appointments for Each Doctor--------------
select Doctors.name ,sum(billing.amount) as [Total Bills] from Doctors inner join Billing
on Doctors.doctor_id=Billing.doctor_id
where Billing.status not like 'Cancelled'
group by Doctors.name

---------12. Find Patients Who Have Not Paid Their Bills (Pending Bills)select Patients.name,Billing.bill_date,Billing.amount,Billing.status  from Patients inner join Billing
on Billing.doctor_id=Patients.patient_id
where Billing.status='Pending'

---------13.Get a List of Patients Who Have Missed (Cancelled) Appointments
select Patients.name,Appointments.appointment_date,Appointments.appointment_time,Appointments.status  from Patients inner join Appointments
on Appointments.patient_id=Patients.patient_id
where Appointments.status like'Cancelled'

--------14.Calculate the Average Bill Amount for All Patients-
select Patients.name,Billing.bill_date,Billing.amount,Billing.status  from Patients inner join Billing
on Billing.doctor_id=Patients.patient_id
where Billing.status='Pending'

------15.Get Total Appointments Per Day
select format(appointment_date,'dddd'),format(appointment_date,'MMMM'),format(appointment_date,'yyyy'),count(appointment_id) from Appointments
group by format(appointment_date,'dddd'),format(appointment_date,'MMMM'),format(appointment_date,'yyyy')

------16. Find Patients Who Have Not Visited in the Last 6 Months
select Patients.name ,Appointments.appointment_date from Patients inner join Appointments
on Patients.patient_id=Appointments.patient_id
where Appointments.appointment_date < DATEADD(month,-6,cast('2024-12-14' as date))

---------17.Calculate Total Revenue by Month
select FORMAT(bill_date,'yyyy')as [Year],FORMAT(bill_date,'MMMM')as [Month],sum(amount) from Billing
where status='Paid'
group by FORMAT(bill_date,'yyyy'),FORMAT(bill_date,'MMMM')

-----18. Find the Most Frequent Patients (Top 3 Patients Based on Number of Appointments

select top 3  Patients.name,count(Appointments.patient_id) as appointment_count from Patients inner join Appointments
on Patients.patient_id=Appointments.patient_id
